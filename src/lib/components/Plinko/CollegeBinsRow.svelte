    ...
    <div class="flex gap-[1%]" style:width={`${($plinkoEngine.binsWidthPercentage ?? 0) * 100}%`}>
      {#each Array($rowCount + 1) as _, binIndex}
        <div
          ...
          style:background-color={binColorsByRowCount[$rowCount as keyof typeof binColorsByRowCount].background[binIndex]}
          style:--shadow-color={binColorsByRowCount[$rowCount as keyof typeof binColorsByRowCount].shadow[binIndex]}
        >
